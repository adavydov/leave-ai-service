...
def _upstream_error_to_issue_and_status(err: Exception, step: str):
    status_code = int(getattr(err, "status_code", 0) or 0)
    message = str(err or "").strip()
    low = message.lower()

    if "timeout" in low or isinstance(err, TimeoutError):
        return 504, make_upstream_issue(
            code="anthropic_timeout",
            message="Превышено время ожидания ответа AI-сервиса.",
            source=step,
            category="network",
        )
    ...
    if status_code == 429:
        return 503, make_upstream_issue(
            ...
            message="AI-сервис временно перегружен (rate limit).",
            source=step,
            category="rate_limit",
        )
    ...
    return 502, make_upstream_issue(
        ...
        message=message or f"Ошибка внешнего AI-сервиса ({type(err).__name__})",
        source=step,
        category="network",
    )
...
def _http_upload_error_to_contract(exc: HTTPException) -> tuple[int, dict[str, Any]]:
    code = "pdf_invalid_type" if exc.status_code == 400 else "pdf_too_large" if exc.status_code == 413 else "upload_error"
    message = str(exc.detail) if exc.detail else "Ошибка загрузки файла."
    ...
    issue = make_upstream_issue(
        ...
        category="document",
        severity="error",
        hint="Загрузите корректный PDF и повторите попытку.",
    )
    decision = build_decision([issue])
    trace = build_trace("", {}, {})
...
async def api_extract(file: UploadFile = File(...)):
    try:
        ...
        status, payload = _http_upload_error_to_contract(e)
